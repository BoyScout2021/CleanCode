...
* 클래스를 생성할 때는 컨테이너에서 파생해야 되며 컨테이너가 요구하는 다양한 생명주기 메서드 제공해야 한다. 즉, 너무 무겁다(heavy).
...
횡단 관심사는 책 내용도 좋지만 이 [글](https://jojoldu.tistory.com/71)을 읽으면 더 잘 이해하실 수 있을 겁니다. 
...
* 하지만 코드가 상당히 많음 제법 복잡하다. 다시 말해서, 프록시를 사용하면 깨끗한 코드를 작성하기 어렵다. 또한 프록시는 (진정한 AOP 해법에 필요한) 시스템 단위로 실행 '지점'을 명시하는 메커니즘도 제공하지 않는다.
...
EJB3는 **XML 설정 파일**과 **자바 5 애너테이션 기능**을 사용해 횡단 관심사를 선언적으로 지원하는 스프링 모델을 따른다.
...
관심사를 모듈로 분리한 POJO 시스템은 기민함을 제공한다. 이런 기민함 덕택에 구조에 대한 결정을 결정하기 제일 적절한 때까지 최대한 늦츨 수 있다. 반면에 결합도가 높은 시스템이라면 기민하게 개발하는 것이 불가능하므로 애초에 먼저 결정해야 하는 단점이 있다.  
